    ...
    <div className="pt-20 bg-slate-50 min-h-screen">
      ...
      <section className="relative py-24 overflow-hidden mb-20 text-white">
        ...
        <div className="max-w-7xl mx-auto px-4 lg:px-8 relative z-10 text-center">
          <h1 className="text-5xl font-bold mb-6 text-white text-white">Execution Plans</h1>
          <p className="text-slate-300 text-xl max-w-2xl mx-auto">Strategic technology bundles designed to take your business to the next level.</p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-8 pb-24">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          {packages.map((pkg, i) => (
            ...
            >
              {pkg.highlight && (
                <div className="absolute -top-5 left-1/2 -ml-20 bg-primary text-white text-[10px] tracking-widest uppercase font-bold px-8 py-2 rounded-full shadow-lg">
                  Most Popular
                </div>
              )}
              ...
              <Link 
                to="/contact" 
                className={`w-full py-4 rounded-2xl font-bold text-center transition-all ${pkg.highlight ? 'teal-gradient text-white shadow-xl shadow-primary/20 hover:scale-105 active:scale-95' : 'bg-secondary text-white hover:bg-opacity-95'}`}
              >
                Inquire Now
              </Link>
            </motion.div>
        ...
        <div className="mt-20 p-8 rounded-3xl bg-secondary text-white flex flex-col md:row items-center justify-between gap-8">
          ...
          <Link to="/contact" className="px-8 py-4 bg-primary text-white rounded-xl font-bold hover:scale-105 transition-transform whitespace-nowrap">
            Build Custom Plan
          </Link>
        </div>
      </div>
